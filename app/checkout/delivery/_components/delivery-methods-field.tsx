...
function DeliveryMethodAccordionField({
  ...
  const { updateState, state } = useMultiStep();
          ...
          ({ deliveryMethod, shippingPrice }) => (
            ...
            >
              <AccordionRadioTrigger
                ...
                onClick={() => {
                  // update the delivery state to update the product summary
                  // updateState({
                  //   delivery: {
                  //     ...state.delivery,
                  //   }
                  ...
                  setValue(
                    "isDeliveredToHome",
                    Boolean(deliveryMethod.isHomeDelivery)
                  );
                  setValue(
                  ...
                  setValue(
                    ...
                    deliveryMethod.formBlocks?.map(
                      ({ title, description }) => ({
                        ...
                        answer: isCheckbox(deliveryMethod) ? "false" : "",
                      })
                    ) || []
                  );
                }}
...
const isCheckbox = (deliveryMethod: DeliveryMethod) =>
  deliveryMethod.formBlocks?.[0].blockType === "multiple-choice-block" &&
                ...
                <FormControl>
                  ...
                  >
                    <FormItem className="flex items-center gap-3 space-y-0 space-x-0">
                      ...
                      <FormLabel // Use Label here instead of FormLabel, so that the color doesn't change when errors are present
                      >
